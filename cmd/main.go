...
func serverSetup(options *Options, logger *zap.Logger) {
	...
	S3CSIDriver, err := csiDriver.NewS3CosDriver(options.NodeID, options.Endpoint, s3client.NewObjectStorageSessionFactory(), mounter.NewMounterFactory(), statsUtil, mounterUtil)
	if err != nil {
